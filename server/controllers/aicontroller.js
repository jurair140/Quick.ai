    ...
    const { data } = await axios.post(
      ...
      formData,
      {
        headers: {
          ...
          ...formData.getHeaders(), // ✅ required in Node
        },
...
export const removeImageBackground = async (req, res) => {
  try {
    ...
    if(image.size > 10485760){
        return res.json({success: false, message:"image size is higher than allowed size."})
    }
...
export const resumeReview = async (req, res) => {
  try {
    ...
    const pdfData = await (dataBuffer)

    const prompt = `Review the following resume and provide constructive feedback on its strengths, weakness, and area for improvement. ResumeContent:\n\n${pdfData.text}`
