...
import { NavLink } from 'react-router-dom'
    ...
    const navItems = [
        ...
        {to:'/ai/generate-images', label: 'Generate Images', Icon: Hash},
...
const Sidebar = ({sidebar, setSidebar}) => {
  ...
  return (
    <div className={`w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute top-14 bottom-0 ${sidebar ? 'translate-x-0 z-20' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
      ...
      <div className='w-full border-t border-gray-200 p-4 px-7 flex items-center justify-between'>
            <div className='flex gap-2 items-center cursor-pointer' onClick={()=>openUserProfile()}>
                <img src={user.imageUrl} className='w-8 h-8 rounded-full' alt="user-avatar" />
                <div>
            ...
            <LogOut onClick={()=>signOut()} className='w-4.5 4.h-5 text-gray-400 hover:text-gray-700 transition cursor-pointer'/> 
      </div>
